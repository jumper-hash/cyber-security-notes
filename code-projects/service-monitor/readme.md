# service-monitor
  ...
  ## Features
    -Service Health Monitoring: Continuously checks the status of a predefined list of systemd services.
    -Automated Recovery: Automatically attempts to restart services that are in an inactive or failed state.
    -Dynamic Loop Control: Interactive prompt to set the monitoring interval (in seconds) or run as a one-time audit.
  ...
  ## Configuration
    -Service Customization: To monitor different services, you must manually modify the services array at the beginning of the script.
    -How to Edit: Open the file in any text editor (e.g., nano or vim) and update the list:
    services=( "your-service.service" "another-one.service" )
    -Log Path: The default log directory is set to /var/log/service-monitor/. This can also be adjusted by changing the logDir variable in the header section.
