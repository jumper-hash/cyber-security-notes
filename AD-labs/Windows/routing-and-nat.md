#Windows-Server-Edge-Routing-and-NAT-Infrastructure
  ...
  ##Technical-Troubleshooting-(The "NAT Nuances")
    ...
    Resolution: Re-assigned WAN as 'Public' with "Enable NAT" flag; set LAN as 'Private'.
    Issue 2: Ping/Curl failure despite correct IP configuration
    Root Cause: Windows Firewall "Forwarded Traffic" filtering
    Nuance: Even with active NAT, the Windows Firewall often drops packets traversing the server if they don't match specific Inbound/Outbound rules or if the "Public" profile is too restrictive.
    Resolution: Diagnostic bypass via Set-NetFirewallProfile -Enabled False to isolate the issue, followed by proper Interface/Forwarding enabling.
    Issue 3: Latency in packet delivery and DNS timeouts
    Root Cause: IPv6 (AAAA) lookup precedence
